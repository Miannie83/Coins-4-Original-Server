...
while {true} do {
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("Entering Trader, Enhanced Safe Zone Enabled"); };
	_thePlayer = player;
	...
	if ( AGN_safeZone_Players_DisableWeaponFiring ) then
	{
		_EH_Fired = _thePlayer addEventHandler ["Fired", {
			systemChat ("Your weapon will not fire in a Trader City Area");
	...
	if ( AGN_safeZone_Vehicles_DisableMountedGuns ) then
	{
		while { !canBuild } do
		{
			...
			{
				if (AGN_safeZoneMessages) then { systemChat ( "Vehicle guns cannot be fired in traders" ); };
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("Exiting Trader Area - Enhanced Safe Zone Disabled"); };
