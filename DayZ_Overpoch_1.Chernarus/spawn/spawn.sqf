private ["_counter","_vipBase","_vipUID","_findSpot","_isNear","_isZero","_nearPlayers","_position","_seldLoc","_seldRandom","_waitScript"];
...
_waitScript =  [] spawn {
	...
	while {statusSelect < 2} do {
		...
		{_x hideObject true;_x allowDamage false;} count nearestObjects [player,["CAManBase"],80];		
		uiSleep 1;
		if (!dialog) then {
			cutText ["","BLACK OUT",0];
...
statusSelect = 1;
closeDialog 0;
if (spawnSelect != 41) then {waitUntil {haloSelect != -1};};
statusSelect = 3;
closeDialog 0;
